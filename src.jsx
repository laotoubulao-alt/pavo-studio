...
function App() {
  ...
  return <div className="appShell">
    ...
    {settings && <Settings config={config} provider={provider} setProvider={setProvider} model={model} setModel={setModel} close={() => setSettings(false)}/>} 
  </div>;
}
...
function Settings({ config, provider, setProvider, model, setModel, close }) { return <div className="modalBackdrop" onMouseDown={close}><section className="modal" onMouseDown={e => e.stopPropagation()}><header className="modalHead"><div><small>SERVER-SIDE PROVIDERS</small><h2>模型与接口</h2></div><button onClick={close}>完成</button></header><p className="securityNote">API Key 只在服务端读取，前端不会获取或保存密钥。</p><div className="providerList">{config.providers.map(item => <button key={item.id} className={provider === item.id ? 'selected' : ''} onClick={() => { setProvider(item.id); setModel(item.model); }}><span className="providerLogo">{item.label[0]}</span><span><b>{item.label}</b><small>{item.capabilities.join(' · ')} · {item.model || '未设置模型名'}</small></span><em className={item.configured ? 'on' : ''}>{item.configured ? <><Check/>已配置</> : '未配置'}</em></button>)}</div><Field label="当前模型名"><input value={model} onChange={e => setModel(e.target.value)} /></Field><p className="envHint">配置文件：项目根目录的 <code>.env</code></p></section></div>; }
